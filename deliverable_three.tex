...
% Environment for use cases introduced during implementation
\newenvironment{ucnew}{
    \noindent\textbf{NEW USE CASE} \par \vspace{0.1cm}
    \begin{quote}
}{
    \end{quote}
    \par\vspace{0.5cm}
...
\phantomsection
\subsection{FR4. Facility doctors allocation}

{\color{red}\sout{The system shall allow the Head of Hospital to macro-allocate doctors within their facility. This shall include assigning doctors to departments in the hospital.}}

\vspace{0.3cm}

The previous formulation has been removed because it is already covered by FR1.8 (Doctor assignment). Hospital-level allocation is therefore not required, as it is implicitly handled through department-level assignment in the updated use cases.
...
\begin{itemize}
    ...
    \item \textbf{FR11.4 \hl{System Administrator authentication}:} The system shall allow an active account with administrative privileges to authenticate through the dedicated administration interface using its email address and password. The system shall deny administration access to inactive accounts and accounts without administrative privileges.
\end{itemize}
\vspace{0.3cm}
...
\end{ucedits}
\phantomsection
\addcontentsline{toc}{subsection}{UC13. Assign Doctor to Department / Hospital}
\hypertarget{UC13}{}
\usecaseheader
  {UC13} % 1: ID
  {Assign Doctor to Department / Hospital} % 2: Titolo
  {To enable the System Administrator or Head of Hospital to assign one or more registered Doctors to a specific department. Hospital assignment is implicit through the department.} % 3: Goal
  {Scope: Hospital structures handling. Level: Primary.} % 4: Scope
  {The user is logged in as a System Admin or Head of Hospital. The target department exists. The Doctors are registered in the System.} % 5: Preconditions
  {The selected Doctors are successfully assigned to the selected department.} % 6: Success
  ...
  {The user selects Doctors and assigns them to a department.} % 9: Trigger
...
\begin{ucflow}
    1 & The user selects one or more Doctors from the set of registered Doctors. \\ \hline
    2 & The user selects the target department. \\ \hline
    3 & The user requests assignment. \\ \hline
    4 & The System checks whether selected Doctors are already assigned to a department. \\ \hline
    5 & The System displays current assignment status (including any existing department assignments). \\ \hline
    6 & The user confirms or modifies the selection. \\ \hline
    7 & The System assigns the Doctors to the selected department and updates the System status. \\ \hline
\end{ucflow}


\begin{ucextensions}
    3a & \textbf{Cancel operation:}\newline 
         1. The user cancels the assignment.\newline 
         2. No changes are made to the System.\newline 
         3. Use case ends. \\ \hline

    6a & \textbf{User does not confirm assignment:}\newline 
         1. The user rejects the proposed assignment or modifies the selection.\newline 
...
\begin{ucedits}
Merged UC13 (Assign Doctor to Hospital) and UC16 (Assign Doctors to Department) into a single use case because hospital assignment is implicit through department assignment.
Moved validity checking of existing assignments before confirmation, so the user is informed of current Doctor allocation status prior to final confirmation.
\end{ucedits}
...
\usecaseheader
  ...
  {A valid, active account with administration access (\texttt{is\_staff}) exists, and the administrator is not authenticated.}
  {An authenticated administrative session is created, and the System Administrator is redirected to the requested administration page.}
  {No administrative session is created, access is denied, and the login form displays an error.}
  {Primary: System Administrator. Secondary: Database.}
  {The System Administrator navigates to the administration interface at \texttt{/admin/}.}
...
\begin{ucflow}
    1 & The System displays the administration login form. \\ \hline
    2 & The System Administrator enters their email address and password. \\ \hline
    3 & The System Administrator submits the login form. \\ \hline
    4 & The System validates that the required credentials were provided. \\ \hline
    5 & The System authenticates the credentials against the stored account. \\ \hline
    6 & The System verifies that the account is active and has staff access. \\ \hline
    7 & The System creates an authenticated server-side session. \\ \hline
    8 & The System redirects the System Administrator to the requested administration page or administration dashboard. \\ \hline
\end{ucflow}

\begin{ucextensions}
    4a & \textbf{Missing credentials:}\newline
         1. One or more required fields are empty.\newline
         2. The System displays a validation error.\newline
         3. Resume at Main Flow Step 2. \\ \hline

    5a & \textbf{Invalid credentials:}\newline
         1. The email address or password is incorrect.\newline
         2. The System denies access and displays an authentication error.\newline
         3. Resume at Main Flow Step 2. \\ \hline

    6a & \textbf{Administrative access not permitted:}\newline
         1. The account is inactive or does not have staff access.\newline
         2. The System denies access and displays an authentication error.\newline
         3. Resume at Main Flow Step 2. \\ \hline
\end{ucextensions}

\begin{ucnew}
UC45 documents the separate Django administration authentication flow exposed
at \texttt{/admin/} and is traced to the newly introduced FR11.4. Although D2
models the System Administrator as a distinct conceptual class, the current
implementation realizes administrative privileges through the base User
account's staff and superuser flags. The unguarded React
\texttt{/admin/dashboard} placeholder is not considered an implemented
administration login. SPID/CIE, account recovery, an explicit cancellation
action, and NFR7 inactivity re-authentication are excluded because they are not
...
\begin{minipage}{\textwidth}
    ...
    \begin{itemize}
        \item \textbf{Using:}
        \textbf{UC42}{Medical record access (Doctor)},
    \end{itemize}
\end{minipage}
\vspace{3em}
...
\begin{minipage}{\textwidth}
    ...
    \begin{itemize}
        \item \textbf{Using:}
        \textbf{UC3}{Create Prescription / Exemption},
        \textbf{UC4}{Upload Examination},
    \end{itemize}
\end{minipage}
\vspace{3em}
...
\begin{minipage}{\textwidth}
    \phantomsection
    \subsection{Class diagram. Appointment entity}
    \begin{center}
        \includegraphics[width=0.8\textwidth]{Class_Diagram/review/appointment.png}
        \captionof{figure}{Class Diagram regarding Appointment entity}
    \end{center}
\end{minipage}
\vspace{3em}
...
\begin{minipage}{\textwidth}
    ...
    It refers to use case \textbf{UC45}, and method \textbf{adminLogin()} from the class \textbf{SystemAdmin} of the class diagram.
